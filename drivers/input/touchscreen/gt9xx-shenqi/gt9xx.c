...
static void goodix_ts_work_func(struct work_struct *work)
{
...
#if GTP_GESTURE_WAKEUP
    if ((gtp_wakeup_flag) &&(DOZE_ENABLED == doze_status || DOZE_WAKEUP == doze_status))
    {               
        ...
        {     
            ...
            else if (0xCC == doze_buf[2])
            {
				...
				if(tap_type[2] == 2)
				{
                	...
                	GTP_INFO("Double click home key to light up the screen!");
					input_report_key(ts->input_dev, KEY_GESTURE_W, 1);
					input_sync(ts->input_dev);
					input_report_key(ts->input_dev, KEY_GESTURE_W, 0);
					input_sync(ts->input_dev);
    ...
    #if GTP_HAVE_TOUCH_KEY
        void gtp_report_key (int i,int status) {
            if (i == 1) {
                input_report_key(ts->input_dev, gtp_key_reverse ? KEY_MENU : KEY_BACK, status);
                GTP_DEBUG("KEY:%d,status:%d",gtp_key_reverse ? KEY_MENU : KEY_BACK,status);
                printk(KERN_ERR "KEY:%d,status:%d\n",gtp_key_reverse ? KEY_MENU : KEY_BACK,status);
            } else if (i == 0) {
                input_report_key(ts->input_dev, gtp_key_reverse ? KEY_BACK : KEY_MENU, status);
                GTP_DEBUG("KEY:%d,status:%d",gtp_key_reverse ? KEY_BACK : KEY_MENU,status);
                printk(KERN_ERR "KEY:%d,status:%d\n",gtp_key_reverse ? KEY_BACK : KEY_MENU,status);
            }
        }
        if (!pre_touch) {
            if(gtp_key_enable){
